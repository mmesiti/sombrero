...
#define _M maxeler_gauge
    ...
    _Complex c12 = conj(_M->c20 * _M->c01 - _M->c21 * _M->c00);
    _Complex c22 = conj(_M->c00 * _M->c11 - _M->c01 * _M->c10);
#undef _M

#define _compl_assign(a,b) \
    (a).re = creal(b);     \
    (a).im = cimag(b);

    _compl_assign(sombrero_gauge->c[0], maxeler_gauge->c00);
    _compl_assign(sombrero_gauge->c[1], maxeler_gauge->c01);
    _compl_assign(sombrero_gauge->c[2],                c02);

    _compl_assign(sombrero_gauge->c[3], maxeler_gauge->c10);
    _compl_assign(sombrero_gauge->c[4], maxeler_gauge->c11);
    _compl_assign(sombrero_gauge->c[5],                c12);

    _compl_assign(sombrero_gauge->c[6], maxeler_gauge->c20);
    _compl_assign(sombrero_gauge->c[7], maxeler_gauge->c21);
    _compl_assign(sombrero_gauge->c[8],                c22);
